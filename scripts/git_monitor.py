...
def run_e2e_tests():
    ...
    try:
        # Ejecutar el script de pruebas E2E sin capturar output
...
def monitor_repo():
    ...
    while True:
        ...
        if current_remote != local_commit:
            ...
            log_metrics(f"Despliegue exitoso - Commit: {current_remote[:8]}")
            log_metrics(f"Lead Time: {lead_time:.2f}s - Cycle Time: {cycle_time:.2f}s")
            # Validar umbrales
